...
/**
 * Handles NGCP packets
 */
class NgcpHandler(vertx: Vertx, config: JsonObject, bulkOperationsEnabled: Boolean) : Handler(vertx, config, bulkOperationsEnabled) {
    ...
    init {
        if (bulkOperationsEnabled) {
            config.getJsonObject("ngcp")?.let { ngcp ->
                ngcp.getInteger("bulk_size")?.let { bulkSize = it }
            }
        }
    }
    ...
    override fun onPacket(packet: Packet) {
        ...
        packet.apply {
            protocolCode = ProtocolCodes.NGCP
            payload = run {
